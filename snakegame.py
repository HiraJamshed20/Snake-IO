...
for i in snakearray:
    ID = i["id"]
    body = i["body"]
    item = {"id": ID, "head": body[0]}
    headarray.append(item)
#print(headarray)
...
def keythread():
    ...
    while alive:
        ...
        #if key not in [KEY_RIGHT, KEY_UP, KEY_DOWN, KEY_LEFT]:
        #    key = prevkey
...
def snakethread(win):
    ...
    while True:
        ...
        """snake.insert(0, [snake[0][0] + (int(key) == int(KEY_DOWN) and 1) + (int(key) == int(KEY_UP) and -1), snake[0][1] + (int(key) == KEY_LEFT and -1) + (key == KEY_RIGHT and 1)])
                #snakehead = snake[0]
